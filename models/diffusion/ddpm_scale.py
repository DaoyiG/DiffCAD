...
class LatentDiffusion(DDPM):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        val_statistics = dict()
        tag = "{0}/{1}".format('val', 'scale_diff')
        val_statistics[tag] = results["scale_diff"]

        print("scale diff for {}-batch of val set: {}".format(batch_idx,
              results["scale_diff"]))
    ...
    def log_visuals(self, batch, N=16, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        if sample:
            ...
            # use the discrepancy between the generated depth and the depth gt as the monitor metric
            nocs_difference = (z - x_samples).abs().mean()
            log["scale_diff"] = nocs_difference
